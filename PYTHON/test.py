...
f = open('/opt/sdb/splkdmp/scripts/SEC_%s%s.csv' %(earliestdate,latestdate),'w')
sys.stdout = f

#Server Information
baseurl = 'https://SPLUNKSERVERNAME:8089'
username = 'username'
password = 'password'
...
searchquery = 'index="domaincontrollers" source="WinEventLog:Security" EventCode=4624 Account_Name=*$* earliest=%s latest=%s | sort 0 _time | table _time _raw'  % (args.earliest,args.latest)
